  ...
  const handleBuy = async (item: Listing) => {
    ...
    try {
      ...
      try {
        ...
        alert(`❌ Error validating NFT: ${error.message}`);
        return;
      }
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredAndSortedListings.map((item) => (
            <div key={item._id} className="bg-white rounded-xl shadow-sm border hover:shadow-md transition-shadow duration-200 overflow-hidden">
